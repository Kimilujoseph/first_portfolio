...
function initContactForm() {
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      sendBtn.innerHTML = `Sending... <span style="display:inline-block;animation:spin 1s linear infinite">⟳</span>`;

      setTimeout(() => {
        sendBtn.disabled = false;
        sendBtn.innerHTML = `Send Message <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>`;
        form.reset();
        counter.textContent = '1000';
        showToast('Message sent! Timothy will get back to you soon.', 'success');
      }, 1500);
    });
  }
